...
export function engineFiles(engine: Engine, ctx: ProjectContext): Record<string, string> {
  const prompt = `Compose the ${engine} instruction file for project "${ctx.name}" from this context:\n${JSON.stringify(ctx)}`;
  const body = aiGenerate(prompt, () => engineBody(engine, ctx));
  switch (engine) {
    case "claude":
    ...
    case "copilot":
      return {
        "AGENTS.md": body,
        ".github/copilot-instructions.md": aiGenerate(
